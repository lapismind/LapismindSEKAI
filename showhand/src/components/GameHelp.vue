...
<template>
  <div v-if="open" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" @click.self="emit('close')">
    <div class="max-h-[80vh] w-full max-w-md overflow-y-auto rounded-2xl border border-brand-200 bg-white p-6">
      <h2 class="mb-4 text-lg font-bold text-slate-800">梭哈规则</h2>
      ...
      <section class="mb-4">
        ...
        <ol class="list-inside list-decimal space-y-1.5 text-sm text-slate-600">
      ...
      <section class="mb-4">
        ...
        <p class="text-sm text-slate-600">同花顺 &gt; 四条 &gt; 葫芦 &gt; 同花 &gt; 顺子 &gt; 三条 &gt; 两对 &gt; 一对 &gt; 高牌</p>
