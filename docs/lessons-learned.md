...
## 2026-09-16 三游戏 UI 缺陷修复

- **`rotate: 0deg` 写在 `transform` 声明之外，会让整条 `transform` 被浏览器丢弃**（CSS 语法层错误，构建不报错、测试不报错、肉眼也未必看得出）。abracadawhat 的 `dragon-pop` 关键帧就这样写了，动画从 25% 直接跳到 100%，落定与旋转回正全丢。改关键帧时要把 `rotate` 并进 `transform`。
- **Chromium 对非 sRGB 颜色会把计算值原样返回**：`getComputedStyle(el).backgroundColor` 拿到的是 `oklch(0.25 0.028 249)` 而不是 `rgb(...)`；`canvas` 的 `fillStyle` **不接受 oklch**，赋值失败会**静默保留上一次的值**，于是所有颜色都读成 `#000000`，看起来像"整个页面全黑"。断言颜色要么自己实现 oklch→sRGB，要么别假定计算值一定是 rgb。
- **验证脚本读状态的时机太早会制造假故障**：本轮改动让聊天面板在 WS 握手完成前显示"○ 连接中…"，脚本在 600ms 时读到的就是这个**正确**状态，差点被记成"新加的连接状态是坏的"。等待类断言要给足时间窗口，并区分"尚未发生"与"永远不会发生"。
- **"文字的可见性"不会被打包器、类型检查或单测发现**：白卡上的 `text-white` 标题存在了很久，构建全绿、测试全绿。做令牌迁移时只核"有没有裸 hex"是不够的——**底色和字色要成对核**（`bg-white` + `text-white` 这类组合是静默失效）。
- **共享包的"深浅两套"必须逐块核，不能只核看起来像主题入口的那一块**：`AuthBadge` 的 `.is-dark` 覆盖了徽章本体（名称/次要文字/幽灵按钮），却漏了 `.lk-modal` / `.lk-form input` / `.lk-mask`，于是引了深色主题的海龟汤在注册/登录时弹出一张**白底表单**。判断依据是"这个选择器在深色下会不会露出来"，不是"它看起来像不像主题的一部分"。
- **`npx wrangler login` 在仓库根执行会把 wrangler 装进 npm 缓存**（本轮装到 4.132.0），与项目 `devDependencies` 锁定的版本（4.123.0）不一致；登录本身是全局生效的，但版本差异会带来 `missing Oauth scopes: websearch.run` 这类告警。恢复部署能力时应在**项目目录**里跑。
- **design-kit 的"可读性下限"这类措辞要算过再信**：README 写 "600 = 浅色底上白字按钮的可读性下限"，实算白字/`brand-600` = **4.16:1**，而 AA 对正文（含 16px 粗体按钮，豁免要 ≥24px）要求 4.5:1——即**所有字号都不达标**，不是只有小字号。文档里的合规声称同样属于"记录可以引用，结论要自己验"。
- **"等 N 条消息就收工"的探针会给出假阴性，而且本地看不出来**：同一个 WS 探针脚本，本地收到 3 条 `game_state` 才拿到配置生效后的状态；生产上前两条是入房广播、第三条才是配置广播，脚本在第 2 条就 resolve 了，读到 `maxPlayers: 8`，差点得出"服务端没强制"的错误结论。等待类断言要**取最后一条状态或等一个明确的收敛条件**，不能按消息条数计数；并且**同一个脚本要在生产上也跑一次**才能暴露这类差异。
- **生产 `/ws` 需要会话，裸 WS 探针连不上**（`index.js` 里 `if (pathname === '/ws' && env.SESSION_SECRET)` 是门禁）：要探生产就在页面上下文里 `new WebSocket(...)`，浏览器会自动带上 cookie；Node 的 `WebSocket` 没有自定义 header 的口子，拿不到这个会话。

## 2026-09-16 海龟汤投票（🍎）三阶段

- **pinia store：声明了、hydrate 了，但忘了放进 return，组件取到 `undefined`。** 表现是组件渲染到一半**整块消失**（只剩一个空的父容器），而构建绿、单测绿、`pageerror` 也是 0——Vue 把这类渲染错误记进 `console.error` 而不是 `pageerror`，只监听 pageerror 会完全看不到。回归断言必须把「声明 / 赋值 / 导出」当成三件**分别**校验的事。
- **固定 `sleep` 等广播，在本地永远过、在生产必然出假故障。** 同一个投票脚本本地 27 项全绿，生产首跑 3 条失败，查下来全是"某一页还没收到广播"。等待要轮询到**条件成立**（文案出现/元素可见），不能按秒数。这是同一类坑在本仓库的第三次记录（前两次：聊天面板连接状态、AI 入口探针），**别再写 sleep**。
- **按显示名定位元素在本地可靠、在生产不可靠。** 生产接了 auth，游客身份会把玩家在大厅输入的名字覆盖掉，于是 `get_by_role("button", name="玩家三")` 在生产什么也找不到。可点元素要挂 `data-testid`（出包早就是这么做的），锚点与显示名解耦。
- **"从 X 起算 N 个字符内出现 Y"这种范围断言会被无关改动撑爆。** 给按钮加一个 `data-testid` 属性就把 200 字符的窗口挤出去，失配信息还只说"找不到按钮"。要定位元素就按稳定的锚（testid）切片，别用字符距离。
- **`networkidle` 不能当导航成败的判据。** 这些站挂着常驻 WebSocket、字体按 unicode-range 分片按需加载，再叠加代理，45 秒也等不到"网络静默"。实测同一份验收脚本连跑两次，在**不同**站点各假失败一次（blog 与 soup），而同一时刻 curl 两个站都是 200。**验收脚本给出假 FAIL 比漏报更糟**——会让人开始无视结果，真出问题时也照过。改为 `domcontentloaded` 判成败、后续等待尽力而为。
- **`net::ERR_ABORTED` 是浏览器主动取消，不是资源缺失。** 媒体预加载（blog 的 `music/*.mp3`、`IntroOverlay` 的 `intro-*.webp`）都会出现，实测这些 URL 直接 curl 都是 200。验收脚本要把它们从"失败请求"里分出来，否则每次验收都要重新判断同样几条噪音。
- **`page.screenshot()` 在生产可能卡在 "waiting for fonts to load"**：截图不能进判据链，要么设 timeout + 吞掉异常，要么别在生产流程里截图。
- **把"默认值"当成入口来关：只禁用/隐藏 UI 是假关闭。** AI 主持原本是 store 的 ref、store 的 reset、Worker 的归一化、四个组件的 prop default 共三处默认值——只藏按钮，新建房间照样落到 AI。**验证方式是直接用协议层探针硬发非法值**（冒充房主发 `mode:'ai'`），看服务端认不认；这比点 UI 有说服力得多。
- **关掉一个功能要连它的"支线"一起查。** 关 AI 主持时，`handleAIHint` 是另一条独立触发、同样烧 token 的路径（主持人随时可点），不一起关就只关了一半。判断依据是"这个开关的真实代价是什么"，不是"名字里有没有 AI"。


## 2026-09-17 从过期工作树部署（差点回退别人 11 个提交）

- **部署前必须先 `git status -sb` 确认与远程的关系，「本地跑完测试」不等于「本地是最新的」。** 本轮在同一个会话里接着做博客文章，部署前顺手一查：`main...origin/main [ahead 1, behind 11]`——另一台机器在这期间推了 11 个提交（海龟汤投票服务端 + 面板、abracadawhat 三处缺陷修复、lobby-kit 的 AuthBadge 迁移）。**若直接部署，这 11 个提交的功能会被整块回退**，而且是"构建成功、验收脚本也通过"式的静默回退：脚本只探关键路径，探不出一个功能消失了。部署产物应当等于仓库 HEAD，跑测试之前先对齐 HEAD。
- **跨设备的会话会"看起来还在继续"、实际已经掉队。** 这个会话跨了几天，中间另一台机器推进了 15 个提交；上一次交互里报出的部署版本号、`未 push` 的判断、乃至"设计语言还没收口"的认知，全部过期。**接手时的第一条命令就该是 `git fetch` + 看 `CURRENT.md`**（`AGENTS.md` 写了这条，本轮漏读，代价是要靠一次巧合的 `git status` 才发现）。判断"我的认知有没有过期"的成本极低，代价极高。
- **`docs/agent/scripts/playwright-verify-deploy.py` 这类验收脚本能挡住"站点挂了"，挡不住"功能被回退"。** 它探的是首页 200 / 字体 / 表情资源这类关键路径。要确认某个功能真的在线，得**探那个功能自己的产物**：本轮用"线上 JS bundle 里有没有 `投票` / `VotePanel` 字符串"来确认海龟汤投票没被回退，比任何页面级冒烟都直接。

## 2026-09-19 「先 pull 再开工」漏执行的又一次实锤（这次坑在开工读档，不是部署）

- **2026-09-17 那条的教训写在部署前，这次证明它在开工时就该执行。** 本轮家机开会话直接读档干活：读的 `CURRENT.md` 是上一轮的（司机机 9-17 已推了封面改版 + Projects 列表页重写 + ConnectionBanner 等 42 个文件的改动），差点对着过期认知动手。`sync.ps1 pull` 一下就拉到 `cdbd8ce..f671998` 一整批新提交——**不 pull 连"接力棒过期了"这件事本身都看不出来**，因为过期的 `CURRENT.md` 读起来跟新鲜的没有区别。
- **规则已从"纪律"升级为"流程"**：`AGENTS.md` 的会话启动清单把 `.\scripts\sync.ps1 pull` 列为第 1 步（先 pull，再读本文件和 `CURRENT.md`，否则读到的规范和接力棒都可能是旧的）。两台机器共用 main 的仓库里，**pull 不是收尾 ship 的镜像动作，是读档的前置**。
