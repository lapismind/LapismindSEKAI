...
with sync_playwright() as playwright:
    ...
    for viewport in ({"width": 1280, "height": 900}, {"width": 390, "height": 844}):
        ...
        achievement_summary = dialog.locator("summary").filter(has_text="其余 1 个成就")
        stats_summary = dialog.locator("summary").filter(has_text="查看完整统计")
        ...
        expect(dialog_rematch).to_be_visible()
        expect(dialog_return).to_be_visible()
        for control in (achievement_summary, stats_summary, dialog_rematch, dialog_return):
        ...
        expect(achievement_summary).to_be_focused()
        page.keyboard.press("Enter")
        expect(collapsed).to_have_attribute("open", "")
        page.keyboard.press("Space")
        expect(collapsed).not_to_have_attribute("open", "")
        page.keyboard.press("Tab")
        expect(stats_summary).to_be_focused()
        page.keyboard.press("Enter")
        expect(stats_summary.locator("xpath=..")).to_have_attribute("open", "")
        page.keyboard.press("Space")
        expect(stats_summary.locator("xpath=..")).not_to_have_attribute("open", "")
        page.keyboard.press("Tab")
        ...
        sent_before_rematch = page.evaluate("window.__b5Sent.length")
        dialog_rematch.click()
        expect(dialog).to_be_visible()
        assert page.evaluate("window.__b5Sent.length") == sent_before_rematch + 1
        assert page.evaluate("window.__b5Sent.at(-1).payload") == {}
        expect(dialog_return).to_be_visible()
        dialog_rematch.click()
        expect(dialog).to_be_visible()
        assert page.evaluate("window.__b5Sent.length") == sent_before_rematch + 2

        page.evaluate("window.__b5SetMatch()")
        page.evaluate("window.__b5SetHost(false)")
        expect(dialog.get_by_test_id("game-over-dialog-actions").get_by_role("button", name="结算详情内再来一局")).to_have_count(0)
        expect(dialog.get_by_test_id("game-over-dialog-actions")).to_contain_text("等待房主再来一局")
        expect(dialog.get_by_role("button", name="结算详情内返回大厅")).to_be_visible()

        page.evaluate("window.__b5SetHost(true)")
        close = page.get_by_role("button", name="关闭比赛结算详情")
        close.focus()
        expect(close).to_be_focused()
