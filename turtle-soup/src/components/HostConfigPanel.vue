...
<template>
  <div class="flex flex-col gap-3 rounded-xl border border-line bg-raised/60 p-4">
    <div class="flex items-center justify-between">
      <span class="text-sm font-semibold text-ink">主持模式</span>
      <div class="flex gap-1 rounded-lg bg-surface p-1">
        ...
        <button
          type="button"
          class="rounded-md px-3 py-1 text-xs font-semibold transition"
          :class="mode === 'human' ? 'bg-brand-500 text-white' : 'text-ink-soft hover:text-ink'"
    ...
    <div class="flex items-center justify-between">
      ...
      <div class="flex flex-wrap gap-1">
        <button
          ...
          :class="maxPlayers === 1 ? 'bg-brand-500 text-white' : 'bg-surface text-ink-soft hover:text-ink'"
        ...
        <button
          ...
          type="button"
          class="h-7 w-7 rounded-md text-xs font-semibold transition"
          :class="maxPlayers === n ? 'bg-brand-500 text-white' : 'bg-surface text-ink-soft hover:text-ink'"
    ...
    <div class="flex items-center justify-between">
      <span class="text-sm font-semibold text-ink">问题次数限制</span>
      <div class="flex gap-1">
        <button
          type="button"
          class="rounded-md px-3 py-1 text-xs font-semibold transition"
          :class="questionLimit === null ? 'bg-brand-500 text-white' : 'bg-surface text-ink-soft hover:text-ink'"
        ...
        <button
          ...
          :class="questionLimit === n ? 'bg-brand-500 text-white' : 'bg-surface text-ink-soft hover:text-ink'"
    ...
    <p v-if="questionLimit !== null" class="text-xs text-muted">
