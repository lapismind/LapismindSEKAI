...
<template>
  <div class="flex flex-col gap-2 rounded-xl border border-sky-700/60 bg-sky-900/30 p-3">
    ...
    <!-- 待判定的问题 -->
    <div v-if="pending" class="rounded-lg bg-raised/80 px-3 py-2 text-sm text-ink">
      <div class="mb-1 text-xs text-ink-soft">{{ askerName(pending.from) }} 提问：</div>
      ...
      <div class="mt-2 flex flex-wrap gap-1.5">
        ...
        <button type="button" class="rounded-md bg-neutral px-3 py-1 text-xs font-bold text-white" @click="submit(JUDGE.IRRELEVANT)">无关</button>
      </div>
    </div>
    <div v-else class="text-xs text-muted">等玩家提问…</div>
  </div>
</template>
